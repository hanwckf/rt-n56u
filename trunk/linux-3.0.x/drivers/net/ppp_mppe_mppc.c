	     ...
	     const unsigned int len, unsigned char *out)
{
	...
	setup_sg(sg_in, in, len);
	setup_sg(sg_out, out, len);
	if (crypto_blkcipher_encrypt(&desc, sg_out, sg_in, len) != 0)
		printk(KERN_WARNING "%s(): crypto_blkcipher_encrypt failed\n", __func__);
}
...
mppe_alloc(unsigned char *options, int opt_len, int comp)
{
	...
	state->sha1_digest = kzalloc(digestsize, GFP_KERNEL);
...
int __init mppe_module_init(void)
{
    ...
    sha_pad = kzalloc(sizeof(struct sha_pad), GFP_KERNEL);
    if (sha_pad == NULL)
	return -ENOMEM;
    /* ... initialize them */
