...
//			NdisZeroMemory(pEntry->PER, sizeof(UCHAR) * MAX_STEP_OF_TX_RATE_SWITCH);//not used
	...
	if (TxTotalCnt <= 15)
	{
		...
		while (idx < pTable[0])
		{
			...
			else if ((pCurrTxRate->CurrMCS == MCS_23) && (pCurrTxRate->ShortGI == GI_800))	//we hope to use ShortGI as initial rate
			{
				MCS23 = idx;
			}
...
//						pEntry->TxQuality[UpRateIdx] --;    // may improve next UP rate's quality
	...
	if (pEntry->CurrTxRateIndex != CurrRateIdx && pEntry->LastSecTxRateChangeAction == 1)//ys
	{
		...
		pEntry->LastSecTxRateChangeAction = 1; // rate UP
//clean the memory after the new mcs is verified			NdisZeroMemory(pEntry->TxQuality, sizeof(USHORT) * MAX_STEP_OF_TX_RATE_SWITCH);
