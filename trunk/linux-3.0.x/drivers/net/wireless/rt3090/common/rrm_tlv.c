...
#define MIN(_x, _y) ((_x) > (_y) ? (_x) : (_y))
	...
	for (loop = 0; loop < pAd->ScanTab.BssNr; loop++)
	{
		...
		if (BssMatch)
		{
			...
			BssidInfo.field.SepctrumMng = pBssEntry->CapabilityInfo & (1 << 8);
			BssidInfo.field.Qos = pBssEntry->CapabilityInfo & (1 << 9);
			BssidInfo.field.APSD = pBssEntry->CapabilityInfo & (1 << 11);
			BssidInfo.field.RRM = pBssEntry->CapabilityInfo & RRM_CAP_BIT;
			BssidInfo.field.DelayBlockAck = pBssEntry->CapabilityInfo & (1 << 14);
			BssidInfo.field.ImmediateBA = pBssEntry->CapabilityInfo & (1 << 15);
