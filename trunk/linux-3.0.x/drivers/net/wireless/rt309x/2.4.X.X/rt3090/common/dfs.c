...
//Roger Said reserve it. Because the new chip maybe use the same interrupt for DFS and Carrier Detection.
// And the only way to classify the source of interrupt is the Tone Radar Status variable.
	...
	if (pAd->CommonCfg.CarrierDetect.CD_State == CD_NORMAL)
	{
		...
		if (pAd->CommonCfg.CarrierDetect.recheck == 0)
		{
			...
			pAd->CommonCfg.CarrierDetect.CD_State = CD_SILENCE;
#ifdef RELASE_EXCLUDE
// After checking with Roger, we remove the recheck2
#endif // RELASE_EXCLUDE //
