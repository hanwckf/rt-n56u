	...
	OUT	VOID *pbuf)
{
	...
	//TODO: integrate WSC_V2_SUPPORT and IWSC_SUPPORT
#ifdef WSC_V2_SUPPORT
	if (pWscControl->WscV2Info.bEnableWpsV2)
	{
	...
	if (pWscControl->WscMode == WSC_PIN_MODE)
	{
		ConfigMethods &= 0x200F;
		if (pAdapter->StaCfg.IWscInfo.bLimitedUI)
		{
			ConfigMethods &= 0xFEFF;
		}
		else
		{
			ConfigMethods |= 0x0100;
		}
	}
	else
	{
		ConfigMethods &= 0x02F0;
	}
#endif // IWSC_SUPPORT //
...
#ifdef SAMSUNG_EASY_CONFIG
	if (pWscControl->EasyConfigInfo.bRaAutoWpsAp &&
		!pWscControl->bWscTrigger)
	{
		//WPS Vendor Extension
		NdisMoveMemory(VendorExt, RALINK_OUI, 3);
		NdisMoveMemory(VendorExt + 3, pWscControl->EasyConfigInfo.WpsEncryptPIN, 12);
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAdapter)
	 {
		if ((pAdapter->StaCfg.WscControl.WscProfile.ProfileCnt == 0) ||
			((pAdapter->StaCfg.WscControl.WscConfStatus == WSC_SCSTATE_CONFIGURED) && (pAdapter->StaCfg.BssType == BSS_ADHOC)))
			WscCreateProfileFromCfg(pAdapter, STA_MODE | REGISTRAR_ACTION, pWscControl, &pWscControl->WscProfile);
...
#ifdef CONFIG_STA_SUPPORT	 	
	 	  ||  ((pAdapter->OpMode == OPMODE_STA) && (pAdapter->StaCfg.BssType == BSS_ADHOC))
#endif // CONFIG_STA_SUPPORT //
		)
...
#ifdef CONFIG_STA_SUPPORT
	if ((pAdapter->OpMode == OPMODE_STA) && (pAdapter->StaCfg.BssType == BSS_INFRA))
	 {
...
#ifdef SAMSUNG_EASY_CONFIG
				AutoProvisionDecodeExtData(pAdapter, pWscControl, pData);				
#endif // SAMSUNG_EASY_CONFIG //
#ifdef WSC_V2_SUPPORT
				if (pWscControl->WscV2Info.bEnableWpsV2)
				{
...
#if 0
	/*
		Atheros ER expact to receives WSC_NACK not WSC_DONE when AP is configured enrollee.
	*/
