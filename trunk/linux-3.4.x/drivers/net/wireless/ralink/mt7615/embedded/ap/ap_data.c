...
//---Add by shiang for debug
    ...
    )
	{
		...
		if (pMbss != NULL)
		{
			pMbss->TransmittedByteCount += pTxBlk->SrcBufLen*100;
			pMbss->TxCount ++;
	...
	while (pTxBlk->TxPacketList.Head)
	{
		...
		*/
...
#endif /* WAPI_SUPPORT */
		{
			...
			if (pMbss != NULL)
			{
				pMbss->TransmittedByteCount += totalMPDUSize*100;
				pMbss->TxCount ++;
...
INT APRxPktAllow(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	if ((pFmeCtrl->FrDs == 1) && (pFmeCtrl->ToDs == 1))
	{
		...
		if (pEntry)
		{
			...
			pWdsEntry->WdsCounter.ReceivedByteCount += pRxBlk->MPDUtotalByteCnt*100;
...
//---Add by shiang for debug
	...
	if (pFmeCtrl->FrDs == 0 && pRxInfo->U2M)
	{
		...
		if (pMbss != NULL)
		{
			pMbss->ReceivedByteCount += pRxBlk->MPDUtotalByteCnt*100;
			pMbss->RxCount ++;
		}
	}
