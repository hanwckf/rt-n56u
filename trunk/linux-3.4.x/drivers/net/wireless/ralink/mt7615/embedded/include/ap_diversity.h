...
#define ADMTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, format,args...)	do{if(atomic_read(&DEBUG_VERBOSE_MODE))	printk( format, ##args);}while(0)
...
{									\
	...
	{                                                   		\
		...
		if (BbpCsr.field.Busy == BUSY)                      	\
		{                                                   	\
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("BBP read R%d=0x%x fail\n", _I, BbpCsr.word));	\
