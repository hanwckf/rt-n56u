...
extern void ez_apcli_link_down_hook(ez_dev_t *ezdev ,
	unsigned long Disconnect_Sub_Reason);
...
enum_group_merge_action_t is_group_merge_candidate_hook(unsigned int easy_setup_capability, 
	ez_dev_t *ezdev, 
	void *temp_bss_entry,
	UCHAR *Bssid);
...
void ez_set_delete_peer_in_differed_context_hook(
	void * ezdev, struct _ez_peer_security_info *ez_peer, BOOLEAN set);
...
BOOLEAN ez_internet_msghandle_hook(ez_dev_t *ezdev, p_internet_command_t p_internet_command);
void ez_custom_data_handle_hook(EZ_ADAPTER *ez_ad, p_ez_custom_data_cmd_t p_custom_data, int length);
...
int  ez_handle_send_packets_hook(ez_dev_t *ezdev, NDIS_PACKET *pPacket);
...
void APTribandRestartNonEzReqAction_hook(EZ_ADAPTER *ez_ad);
...
VOID ez_connection_alllow_all_hook(EZ_ADAPTER *ez_ad);
void ez_stop_hook(
	ez_dev_t * ezdev);

VOID ez_connection_allow_all_hook(EZ_ADAPTER *ez_ad, unsigned char default_pmk_valid);

VOID ez_scan_timeout_hook(ez_dev_t *ezdev);
