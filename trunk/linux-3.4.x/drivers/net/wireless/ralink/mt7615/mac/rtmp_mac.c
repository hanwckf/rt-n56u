...
VOID dump_txinfo(RTMP_ADAPTER *pAd, TXINFO_STRUC *pTxInfo)
{
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxInfo Fields:\n"));
...
VOID dump_tmac_info(RTMP_ADAPTER *pAd, UCHAR *tmac_info)
{
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxWI Fields:\n"));
#ifdef RLT_MAC
	if (pAd->chipCap.hif_type == HIF_RLT)
...
VOID dump_rmac_info(RTMP_ADAPTER *pAd, UCHAR *rmac_info)
{
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("RxWI Fields:\n"));
#ifdef RLT_MAC
	if (pAd->chipCap.hif_type == HIF_RLT)
...
#ifdef RLT_MAC
	if (pAd->chipCap.hif_type == HIF_RLT) {
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): TBD for this function!\n", __FUNCTION__));
	}
#endif /* RLT_MAC */
		...
		for (index = 3; index >= 0; index--)
		{
			...
			for (TxIdx = 0; TxIdx < TX_RING_SIZE; TxIdx++)
			{
				...
				if (!pTxD->DMADONE)
				{
					...
					os_move_mem((PUCHAR)&TxWI, (PUCHAR)pDestTxWI, TXWISize);
					pTxWI = &TxWI;
...
#ifdef RT_BIG_ENDIAN
					RTMPWIEndianChange(pAd, (PUCHAR)pTxWI, TYPE_TXWI);
					os_move_mem((PUCHAR)pDestTxWI, (PUCHAR)pTxWI, TXWISize);
#endif /* RT_BIG_ENDIAN */
				}
			}
		}
...
INT rtmp_mac_fifo_stat_update(RTMP_ADAPTER *pAd)
{
	...
	/* ignore NoACK and MGMT frame use 0xFF as WCID */
		if ((StaFifo.field.TxAckRequired == 0) || (!VALID_UCAST_ENTRY_WCID(pAd, wcid)))
		{
			i++;
			continue;
		}
		...
		if (pAd->MacTab.tr_entry[wcid].PsDeQWaitCnt)
		...
		if (!StaFifo.field.TxSuccess)
		{
			...
			{			
				...
				if(pEntry->PsMode == PWR_ACTIVE)
				{
					...
					{ 
						MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Entry %02x:%02x:%02x:%02x:%02x:%02x Blocked!! (Fail Cnt = %d)\n",
	...
	IN TX_STA_CNT1_STRUC *pStaTxCnt1)
{
	...
	pAd->WlanCounters[0].TransmittedFragmentCount.u.LowPart += pStaTxCnt1->field.TxSuccess;
	pAd->WlanCounters[0].RetryCount.u.LowPart += pStaTxCnt1->field.TxRetransmit;
	pAd->WlanCounters[0].FailedCount.u.LowPart += pStaTxCnt0->field.TxFailCount;
...
VOID NICUpdateRawCounters(RTMP_ADAPTER *pAd)
{
	...
	pAd->WlanCounters[0].FCSErrorCount.u.LowPart += (RxStaCnt0.field.CrcErr); /* >> 7);*/
...
#ifdef STATS_COUNT_SUPPORT
	pAd->WlanCounters[0].FrameDuplicateCount.u.LowPart += RxStaCnt2.field.RxDupliCount;
#endif /* STATS_COUNT_SUPPORT */
	...
	*/
	...
	if (pMac && pAd->chipCap.FlgHwTxBfCap)
	{
...
#endif
		{
			...
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : NDP Sounding and NDPSndRate = %d\n", __FUNCTION__, ndp_rate));
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : BSSID[5] = %x, txwi_n->TxEAPId = %d\n", __FUNCTION__, pAd->CommonCfg.Bssid[5], TxEAPId_Cal));
		}
	}
	...
	os_move_mem(pOutTxWI, &TxWI, TXWISize);
...
VOID write_tmac_info_Data(RTMP_ADAPTER *pAd, UCHAR *buf, TX_BLK *pTxBlk)
{
	...
	*/
	...
	if (pTxBlk->TxSndgPkt == SNDG_TYPE_SOUNDING)
	{
		sounding = 1;
		eTxBf = 0;
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ETxBF in %s(): sending normal sounding, eTxBF=%d\n",
	...
	else if (pTxBlk->TxSndgPkt == SNDG_TYPE_NDP)
	{
		...
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ETxBF in %s(): NDP sounding, eTxBF=%d, ndp_rate = %d\n",
...
VOID write_tmac_info_Cache(RTMP_ADAPTER *pAd, UCHAR *buf, TX_BLK *pTxBlk)
{
	...
	else
	{
		...
		{
			eTxBf = ~(pTransmit->field.eTxBF);
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("ETxBF in AP_AMPDU_Frame_Tx(): invert eTxBF\n"));
		}
		else
#endif	/* MFB_SUPPORT */
			eTxBf = pTransmit->field.eTxBF;
...
INT rtmp_mac_sys_reset(RTMP_ADAPTER *pAd, BOOLEAN bHardReset)
{
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():MACVersion[Ver:Rev=0x%08x]\n",
...
static VOID rtmp_chip_rt_init(RTMP_ADAPTER *pAd)
 {
...
INT rtmp_mac_init(RTMP_ADAPTER *pAd)
{
	...
	/*move from mt_wifi_init*/
	rtmp_chip_rt_init(pAd);
...
INT rtmp_nic_asic_init(RTMP_ADAPTER *pAd)
{
	INT ret = NDIS_STATUS_SUCCESS;
	
	if (RtAsicWaitPDMAIdle(pAd, 100, 1000) != TRUE) 
	{
...
#ifdef CONFIG_AP_SUPPORT
#endif /*RTMP_AP_SUPPORT*/

	if (rtmp_mac_sys_reset(pAd,TRUE) == FALSE)
	...
	RtAsicSetDevMac(pAd, pAd->CurrentAddress, 0x0);

	MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Current MAC: =%02x:%02x:%02x:%02x:%02x:%02x\n",
...
VOID rtmp_chip_info_show(RTMP_ADAPTER *pAd)
{
