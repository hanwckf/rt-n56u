	...
	IN	PRTMP_ADAPTER		pAd)
{
	...
	/* init spin lock */
	NdisAllocateSpinLock(&(pAd->ApCfg.FtTab.FT_KdpLock));
...
//		NdisMoveMemory(pHdr8023->SA, pAd->ApCfg.MBSSID[ApIdx].Bssid, 6);
		...
		skb_reserve(RTPKT_TO_OSPKT(pPktComm), 2); /* 16 byte align the IP header */
		NdisMoveMemory(skb_put(RTPKT_TO_OSPKT(pPktComm), LENGTH_802_3),
						pHdr8023, LENGTH_802_3);

		FrameLen = RT_SIGNAL_STRUC_HDR_SIZE+sizeof(FT_KDP_EVT_HEADER)+EventLen;
		NdisMoveMemory(skb_put(RTPKT_TO_OSPKT(pPktComm), FrameLen),
						pFtKdp, FrameLen);
