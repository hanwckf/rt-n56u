...
/****************************************************************************
	...
	History:
		...
		6. 2010/01/07	Sample Lin
			...
			(2) Add function: Aggregation (RAILNK & AMSDU)
		...
		*/
		...
		else
		{
			...
			/*
				...
				TimeData = 304us in A band for WPA2/AES.
				so medium time = pps * MpduExgTime = 51 * 428 = 21828us

				EX3: data size = 208B, rate = 6Mbps,
				TimeData = 284us in A band for OPEN/NONE.
				so medium time = pps * MpduExgTime = 51 * 408 = 20808us
			*/
			if (BodyLen > 0)
			{
				DataExtraLen = ACM_EncryptExtraLenGet(pAd, pCdb);
