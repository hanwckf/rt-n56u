	...
	IN PSTRING arg)
{
	...
	switch(Cmd)
	{
		case 1: /* insert a STA for RRM Beacon Report request testing. */
			pEntry = MacTableInsertEntry(pAd, StaAddr, 0, TRUE);
	...
	IN LONG Length)
{
	...
	UCHAR SsidLen = 0, MessageToMe=0, BssType = BSS_ANY, Channel;
	USHORT BeaconPeriod = 0;
	UCHAR NewChannel;
	UCHAR DtimCount=0, DtimPeriod=0, BcastFlag=0;
	UCHAR Erp;
	UCHAR SupRate[MAX_LEN_OF_SUPPORTED_RATES], ExtRate[MAX_LEN_OF_SUPPORTED_RATES];
	UCHAR SupRateLen = 0, ExtRateLen = 0;
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Bssid=%02x:%02x:%02x:%02x:%02x:%02x\n",
			pBcnRep->Bssid[0], pBcnRep->Bssid[1], pBcnRep->Bssid[2],
	...
	Idx = BssTableSetEntry(pAd, &pAd->ScanTab, pBcnRep->Bssid, Ssid, SsidLen,
							BssType, BeaconPeriod, &CfParm, AtimWin,
							CapabilityInfo, SupRate, SupRateLen,
							ExtRate, ExtRateLen, &HtCapability,
							&AddHtInfo, HtCapabilityLen, AddHtInfoLen,
							NewExtChannelOffset, pBcnRep->ChNumber,
