...
UCHAR *get_dev_l2profile(RTMP_ADAPTER *pAd)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			{
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("=====GET L2 PROFILE idx: %d =====\n",get_dev_config_idx(pAd) ) );
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("PATH: %s\n", l1profile[get_dev_config_idx(pAd)].profile_path ) );
...
NDIS_STATUS load_dev_l1profile(IN RTMP_ADAPTER *pAd)
{
	...
	if (srcf.Status) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Open file \"%s\" failed, try embedded default!\n", L1_PROFILE_PATH));

		retval = strlen(l1profile_default);
		strncpy(buffer, l1profile_default, buf_size - 1);
	} else {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Open file \"%s\" Succeed!\n", L1_PROFILE_PATH));
	...
	if (retval) {
		...
		if (RTMPGetKeyParameter(profile_index, tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, TRUE)) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Chip=%s\n", tmpbuf));
...
err_out1:
	if ((srcf.Status == 0) && (os_file_close(srcf) != 0)) {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Close file \"%s\" succeed!\n", L1_PROFILE_PATH));
	}
