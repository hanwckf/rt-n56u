...
VOID mt7915_wps_led_init(RTMP_ADAPTER *pAd)
{
	...
	if (IS_MT7915(pAd)) {
		...
		/*Default turn off: GPIO => 1*/
		IdMac = 0x70000000 + 0x40B0;
		RTMP_IO_READ32(pAd->hdev_ctrl, IdMac, &macVal);
		macVal |= BIT6;
		RTMP_IO_WRITE32(pAd->hdev_ctrl, IdMac, macVal);
#else
		/*for colgin setting, use gpio20 as wps led*/
		/*set pinmux to GPIO*/
...
{
#ifdef RTMP_MAC_PCI
	MTWF_LOG(DBG_CAT_HIF, CATHIF_PCI, DBG_LVL_OFF, ("%s::status=0x%x\n",
			 __func__, status));
...
static VOID pci_sw_int_handler(RTMP_ADAPTER *ad, void *hif_chip_ptr)
{
	...
	MTWF_LOG(DBG_CAT_HIF, CATHIF_PCI, DBG_LVL_OFF, ("%s::sw_int_sta=0x%x\n", __func__, sw_int_sta));
...
static VOID mt7915_dump_ser_stat(RTMP_ADAPTER *pAd, BOOLEAN fgDumpAll)
{
	UINT32 reg_tmp_val;
...
static VOID show_muru_local_bsrp_ctrl(struct _RTMP_ADAPTER *pAd, UINT32 base)
{
	...
	if (!err) {
		...
		offset = OFFSET_OF(CMD_MURU_BSRP_CTRL, u4PpduDur);
		addr = subbase + offset;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("| |-(0x%08X) u4PpduDur = %d\n", addr, muru_io_r_u32(pAd, addr)));
...
#endif /* CONFIG_STA_SUPPORT */
	arch_ops->archAutoFallbackInit = MtfAsicAutoFallbackInit;
