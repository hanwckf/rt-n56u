...
UINT16 iwprivPhyStatHelp(char *src, UINT16 limitLen)
{
	...
	totalLen += sprintf(src, " available commands:\n");
	...
	IN RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	os_alloc_mem(NULL, (UCHAR **)&msg, sizeof(CHAR) * (wtbl_max_num * ASSO_MAC_LINE_LEN));
	...
	memset(msg, 0, wtbl_max_num * ASSO_MAC_LINE_LEN);
	...
	for (i = 0; i < wtbl_max_num; i++) {
		...
		if ((IS_ENTRY_CLIENT(pEntry) || (IS_ENTRY_PEER_AP(pEntry) || IS_ENTRY_REPEATER(pEntry)))
			...
			if ((strlen(msg) + ASSO_MAC_LINE_LEN) >= (wtbl_max_num * ASSO_MAC_LINE_LEN))
				break;
	...
	IN INT cmd)
{
...
#ifdef APCLI_SUPPORT
			if (pObj->ioctl_if_type == INT_APCLI) {
				pApCliEntry = &pAd->StaCfg[apcli_idx];
				...
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					...
					apcli_idx, pApCliEntry->CfgSsidLen,
					pApCliEntry->CfgSsid));
	...
	case OID_802_11_AUTHENTICATION_MODE: {
		...
		if (wrq->u.data.length != sizeof(NDIS_802_11_AUTHENTICATION_MODE))
			...
			if (pObj->ioctl_if_type == INT_APCLI) {
				PSTA_ADMIN_CONFIG pApCliEntry = &pAd->StaCfg[pObj->ioctl_if];
	...
	IN INT cmd)
{
	...
	case OID_GET_CPU_TEMPERATURE:{
		...
		RTMP_GET_TEMPERATURE(pAd, band, &temperature);
...
#endif /*DOT11_HE_AX*/
#endif
	case OID_DUT_GET_REJECTED_GROUP:
		{
			UINT32 rejected_group = 0;
			wrq->u.data.length = sizeof(UINT32);
#ifdef DOT11_SAE_SUPPORT
			sae_get_rejected_group(pAd, NULL, NULL, &rejected_group);
#endif
	...
	IN	RTMP_STRING *arg)
{
	...
	pObj->ioctl_if, pAd->ApCfg.MBSSID[pObj->ioctl_if].DtimPeriod));
#else
...
INT	Set_VLANPriority_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (if_type == INT_MAIN || if_type == INT_MBSSID)
		...
		wdev = &pAd->WdsTab.WdsEntry[ifIndex].wdev;
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ifIndex = %d\n", ifIndex));
	}
	else {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Unexpected if_type\n"));
		return FALSE;
...
INT Show_SimSectionUlm_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (Idx == 0 || Idx == 2 || Idx == 3)
		sprintf(fileName, "/etc/mem_dump_ulm%d.txt", Idx);
	else if (Idx == 4)
		sprintf(fileName, "/etc/mem_dump_cache_sram.txt");
	else
		return FALSE;
...
VOID RTMPIoctlStatistics(RTMP_ADAPTER *pAd, RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	sprintf(msg + strlen(msg), "Rx success                      = %lu\n", rxCount);
#ifdef CONFIG_QA
	...
	sprintf(msg + strlen(msg), "Rssi: ");
	for (path_idx = 0; path_idx < len; path_idx++)
		sprintf(msg + strlen(msg), "%d ", rssi[path_idx]);
	sprintf(msg + strlen(msg), "\n");

	sprintf(msg + strlen(msg), "CN Info:				= %d", u2CnInfo);
	sprintf(msg + strlen(msg), "\n");
...
INT	ApCfg_Set_PerMbssMaxStaNum_Proc(
	...
	IN RTMP_STRING *arg)
{
	pAd->ApCfg.MBSSID[apidx].MaxStaNum = (UINT16)os_str_tol(arg, 0, 10);
