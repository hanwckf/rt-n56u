...
#endif
#ifdef MCAST_VENDOR10_CUSTOM_FEATURE
		if ((wdev->channel > 14) && (wdev->rate.MCastPhyMode_5G.field.MODE == MODE_CCK)) {
#else
		if ((wdev->channel > 14) && (wdev->rate.mcastphymode.field.MODE == MODE_CCK)) {
#endif
...
void update_att_from_wdev(struct wifi_dev *dev1, struct wifi_dev *dev2)
{
	...
	wlan_config_set_ba_enable(dev1, wlan_config_get_ba_enable(dev2));
