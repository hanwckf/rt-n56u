		...
		IN	UINT32                              		 length)
{
	...
	if (ieee80211_is_mgmt(mgmt->frame_control)) 
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG80211_PKT: %s Deauth Frame on %d\n", preStr, pAd->LatchRfRegs.Channel));
		}
	...
	if ((pHeader->FC.SubType == SUBTYPE_PROBE_REQ) ||
	 	   ...
	 	   CFG80211_CheckActionFrameType(pAd, "RX", (PUCHAR)pHeader, MPDUtotalByteCnt)))
		{
			...
			{	 
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("pAd->dummy_p2p_net_dev is NULL!!!!!!!!!!!\n")); 
					return FALSE;
				}
				else
#endif /* RT_CFG80211_P2P_CONCURRENT_DEVICE */					
