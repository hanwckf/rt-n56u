	...
	UINT						Data)
{
	...
	if(Data == tdls_insert_entry)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s() - MAX_NUM_OF_CFG_TDLS_ENTRY exceed %d\n", __FUNCTION__,MAX_NUM_OF_CFG_TDLS_ENTRY));
			return FALSE;
		}
...
{	
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("AP Prohibited TDLS, won't send DISCOVER or SETUP RESPONSE \n"));
		return 0;
	}
...
VOID cfg_tdls_rx_parsing(PRTMP_ADAPTER pAd,RX_BLK *pRxBlk)
{
		...
		&& NdisEqualMemory(TDLS_LLC_SNAP_WITH_CATEGORY, pRxBlk->pData, LENGTH_802_1_H + 2))
	{
		...
		if(TDLSType == TDLS_ACTION_CODE_SETUP_REQUEST|| TDLSType == TDLS_ACTION_CODE_SETUP_RESPONSE)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s() - CFG_Tdls_info.TDLSEntry[%d].MacAddr  (%02X:%02X:%02X:%02X:%02X:%02X)\n"
