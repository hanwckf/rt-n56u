		...
		BOOLEAN                                          isReg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[%d] pAd->Cfg80211RegisterProbeReqFrame=%d[%d]\n", 
 ...
 */
	...
	{
		CFG80211DBG(DBG_LVL_OFF, ("80211> Set the channel in ADHOC Mode\n"));
		/* update IBSS beacon */
	...
	VOID						*pData)
{
	...
	if (pIbssInfo->BeaconExtraIeLen > 0)
	{
		...
		else
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG80211 %s: MEM ALLOC ERROR\n", __FUNCTION__));
		}
	...
	VOID						*pData)
{
...
#ifdef CFG_TDLS_SUPPORT
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("[%s] key.MAC = %02x:%02x:%02x:%02x:%02x:%02x \n", __func__, PRINT_MAC(pKeyInfo->MAC)));
	...
	for (i = 0; i < MAX_NUM_OF_CFG_TDLS_ENTRY; i++)
	{
		if (!MAC_ADDR_EQUAL(pCfgTdls->TDLSEntry[i].MacAddr, nullAddr) && MAC_ADDR_EQUAL(pKeyInfo->MAC, pCfgTdls->TDLSEntry[i].MacAddr))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[%s] ===> TDLS_COPY_KEY \n", __func__));
			return TRUE;
		}
	}
#endif /* CFG_TDLS_SUPPORT */
	...
	if (pKeyInfo->KeyType == RT_CMD_80211_KEY_AES_CMAC)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ERROR !! pKeyInfo->KeyId=%d \n",pKeyInfo->KeyId));	
	...
	if (pKeyInfo->KeyType == RT_CMD_80211_KEY_WEP40 || pKeyInfo->KeyType == RT_CMD_80211_KEY_WEP104)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT_CMD_80211_KEY_WEP ==> Id:%d, Type:%d, Len:%d bPair:(%d)\n", 
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_WPAPSK_Proc ==> id:%d, type:%d, len:%d\n", 
...
#endif	
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Install GTK: %d\n", IoctlSec.Alg));
		...
		else
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Install PTK: %d\n", IoctlSec.Alg));
		...
		if ( pAd->StaCfg.wdev.AuthMode == Ndis802_11AuthModeWPANone)
		{
			if (pAd->StaCfg.PairCipher == Ndis802_11Encryption2Enabled)
                        ...
                        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Install ADHOC PTK: %d\n", IoctlSec.Alg));
                        IoctlSec.ext_flags = RT_CMD_STA_IOCTL_SECURTIY_EXT_SET_TX_KEY;
	...
	VOID						*pData)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CFG80211: Connected, disconnect first !\n"));
	}
	else
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CFG80211: No Connection\n"));
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WPS Connection onGoing.....\n"));
	...
	if (pConnInfo->WpaVer == 2)
	{
		if (pConnInfo->FlgIs8021x == TRUE) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WPA2\n"));
			Set_AuthMode_Proc(pAd, "WPA2");
		}
		else 
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WPA2PSK\n"));
	...
	{
		if (pConnInfo->FlgIs8021x == TRUE) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WPA\n"));
			Set_AuthMode_Proc(pAd, "WPA");
		}
		else 
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WPAPSK\n"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("GTK AES\n"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("GTK TKIP\n"));
    ...
    IN VOID 					*pAdCB)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("80211> CFG80211_LostApInform ==> %d\n", 
	...
	IN UCHAR					FlgIsSuccess)
{
	...
	if (pAd->cfg80211_ctrl.FlgCfg80211Scanning == TRUE || 
 	...
 	{
		CFG80211DBG(DBG_LVL_ERROR, ("Abort progressing scan\n"));
...
)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set Sta Default Key: %d\n", Data));
...
)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set Sta MgmtDefault Key: %d\n", Data));
        ...
        IN VOID                                         *pAdCB)
{
	...
	if (bss)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Found %s in Kernel_ScanTable with CH[%d]\n", pApCliEntry->MlmeAux.Ssid, bss->channel->center_freq));
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0))
		cfg80211_put_bss(pWiphy, bss);
#else
		bss->tsf = timestamp;
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Can't Found %s in Kernel_ScanTable & Try Fake it\n", pApCliEntry->MlmeAux.Ssid));
	}
