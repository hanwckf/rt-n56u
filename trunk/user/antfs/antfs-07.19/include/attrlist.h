...
extern int ntfs_attrlist_need(struct ntfs_inode *ni);

extern int ntfs_attrlist_entry_add(struct ntfs_inode *ni,
				   struct ATTR_RECORD *attr);
extern int ntfs_attrlist_entry_rm(struct ntfs_attr_search_ctx *ctx);
 ...
 */
static inline void ntfs_attrlist_mark_dirty(struct ntfs_inode *ni)
{
	if (ni->nr_extents == -1)
