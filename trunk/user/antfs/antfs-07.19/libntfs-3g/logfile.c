 ...
 */
static bool ntfs_check_restart_page_header(struct RESTART_PAGE_HEADER *rp,
					   s64 pos)
{
	...
	if (logfile_system_page_size < NTFS_BLOCK_SIZE ||
		...
		antfs_log_error("$LogFile uses unsupported page size.");
		return FALSE;
	}
	/*
	...
	if (pos && pos != logfile_system_page_size) {
		antfs_log_error("Found restart area in incorrect "
				"position in $LogFile.");
		return FALSE;
	}
	/*
	...
	if (((rp->major_ver != const_cpu_to_sle16(1))
		...
		antfs_log_error("$LogFile version %i.%i is not "
	...
	if (usa_count != le16_to_cpu(rp->usa_count)) {
		antfs_log_error("$LogFile restart page specifies "
	...
	if (usa_ofs < offsetof(struct RESTART_PAGE_HEADER, usn) ||
...
skip_usa_checks:
	...
	if (ra_ofs & 7 || (have_usa ? ra_ofs < usa_end :
	    ra_ofs < offsetof(struct RESTART_PAGE_HEADER, usn)) ||
	...
	if (!ntfs_is_chkd_record(rp->magic) && sle64_to_cpu(rp->chkdsk_lsn)) {
		antfs_log_error("$LogFile restart page is not modified "
 ...
 * require the full restart page.
 */
static bool ntfs_check_restart_area(struct RESTART_PAGE_HEADER *rp)
{
	u64 file_size;
	...
	if (ra_ofs + offsetof(struct RESTART_AREA, file_size) >
		...
		antfs_log_error("$LogFile restart area specifies "
	...
	if (((ca_ofs + 7) & ~7) != ca_ofs ||
	    ra_ofs + ca_ofs > (u16) (NTFS_BLOCK_SIZE - sizeof(u16))) {
		antfs_log_error("$LogFile restart area specifies "
	...
	if ((u32) (ra_ofs + ra_len) > le32_to_cpu(rp->system_page_size) ||
		...
		antfs_log_error("$LogFile restart area is out of bounds "
 ...
 */
static bool ntfs_check_log_client_array(struct RESTART_PAGE_HEADER *rp)
{
	struct RESTART_AREA *ra;
...
check_list:
	...
	return TRUE;
err_out:
	antfs_log_error("$LogFile log client array is corrupt.");
	return FALSE;
}
 ...
 */
static int ntfs_check_and_load_restart_page(struct ntfs_attr *log_na,
					    struct RESTART_PAGE_HEADER *rp,
					    s64 pos,
					    struct RESTART_PAGE_HEADER **wrp,
					    LSN *lsn)
{
	struct RESTART_AREA *ra;
	...
	if (le32_to_cpu(rp->system_page_size) <= NTFS_BLOCK_SIZE) {
		...
		err = ntfs_attr_pread(log_na, pos,
				      le32_to_cpu(rp->system_page_size), trp);
		if (err != le32_to_cpu(rp->system_page_size)) {
			antfs_log_error("Failed to read whole restart page "
					"into the buffer.");
			if (err != -ENOMEM)
				err = -EIO;
			goto err_out;
	...
	if ((!ntfs_is_chkd_record(trp->magic) || le16_to_cpu(trp->usa_count))
	    && ntfs_mst_post_read_fixup((struct NTFS_RECORD *) trp,
					le32_to_cpu(rp->system_page_size))) {
		/*
		...
		if (le16_to_cpu(rp->restart_area_offset) +
			...
			antfs_log_error("Multi sector transfer error "
 ...
 */
bool ntfs_check_logfile(struct ntfs_attr *log_na,
			struct RESTART_PAGE_HEADER **rp)
{
	s64 size, pos;
	LSN rstr1_lsn, rstr2_lsn;
	struct ntfs_volume *vol = log_na->ni->vol;
	u8 *kaddr = NULL;
	...
	if (size < log_page_size * 2 || (size - log_page_size * 2) >>
		...
		antfs_log_error("$LogFile is too small.");
		return FALSE;
	}
	...
	for (pos = 0; pos < size; pos <<= 1) {
		...
		if (ntfs_attr_pread(log_na, pos, NTFS_BLOCK_SIZE, kaddr) !=
		    NTFS_BLOCK_SIZE) {
			antfs_log_error("Failed to read first NTFS_BLOCK_SIZE "
...
is_empty:
		antfs_log_leave("Done.  ($LogFile is empty.)");
		return TRUE;
	}
	if (!rstr1_ph) {
		if (rstr2_ph)
			antfs_log_error("BUG: rstr2_ph isn't NULL!");
		antfs_log_error("Did not find any restart pages in "
 ...
 */
bool ntfs_is_logfile_clean(struct ntfs_attr *log_na,
			   struct RESTART_PAGE_HEADER *rp)
{
	struct RESTART_AREA *ra;
	...
	if (NVolLogFileEmpty(log_na->ni->vol)) {
		antfs_log_debug("$LogFile is empty");
		return TRUE;
	}
	if (!rp) {
		antfs_log_error("Restart page header is NULL");
		return FALSE;
	}
	if (!ntfs_is_rstr_record(rp->magic) &&
	    !ntfs_is_chkd_record(rp->magic)) {
		antfs_log_error("Restart page buffer is invalid");
		return FALSE;
	}
	...
	if (ra->client_in_use_list != LOGFILE_NO_CLIENT &&
		...
		antfs_log_error("The disk contains an unclean file system (%d, "
				"%d).", le16_to_cpu(ra->client_in_use_list),
				le16_to_cpu(ra->flags));
		return FALSE;
	}
	/* $LogFile indicates a clean shutdown. */
	antfs_log_leave("$LogFile indicates a clean shutdown");
	return TRUE;
}
 ...
 */
int ntfs_empty_logfile(struct ntfs_attr *na)
{
	s64 pos, count;
	...
	if (!NAttrNonResident(na)) {
		antfs_log_error("Resident $LogFile $DATA attribute");
		return -EIO;
	}
	...
	while ((count = na->data_size - pos) > 0) {
		...
		if (count <= 0) {
			antfs_log_error("Failed to reset $LogFile");
			if (!count)
				return -EIO;
			return (int)count;
		}
