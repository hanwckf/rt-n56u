	...
	if [ "$caddy_enable" = "1" ] ;then
		...
		if [ ! -f "$caddybin" ]; then
			if [ ! -f "$caddy_dir/caddy/caddy_filebrowser" ]; then
				curl -k -s -o $caddy_dir/caddy/caddy_filebrowser --connect-timeout 10 --retry 3 https://dev.tencent.com/u/dtid_39de1afb676d0d78/p/kp/git/raw/master/caddy_filebrowser
				if [ ! -f "$caddy_dir/caddy/caddy_filebrowser" ]; then
