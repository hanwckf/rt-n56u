...
void process(struct Interface *ifacel, unsigned char *msg, int len, struct sockaddr_in6 *addr, int ipi6_ifindex, int hoplimit)
{
	...
	if (iface == NULL) {
		dlog(LOG_DEBUG, 2, "received packet from unknown interface: %d", ipi6_ifindex);
		return;
	}
