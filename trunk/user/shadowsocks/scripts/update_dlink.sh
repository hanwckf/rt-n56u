...
start_link(){
    ...
    do
		...
		echo "var d_rules = [" > /etc/storage/dlink/dlink.js
		grep -v '^#' /tmp/newdlink.txt | grep -v "^$" >> /etc/storage/dlink/dlink.js
		echo "];" >> /etc/storage/dlink/dlink.js
		logger -t "SS" "已订阅全部节点,退出订阅程序...请手动刷新页面..."
}
				...
				do
					...
					if [ -n "$decode_link" ];then
						get_v2ray_remote_config "$decode_link" "$v2ray_group_tmp"
						 [ "$?" == "0" ] && add_v2ray_servers || logger -t "SS" "检测到一个错误节点，已经跳过！"
					else
						logger -t "SS" "解析失败！！！"
					fi
				done
