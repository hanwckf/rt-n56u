...
/* agf.this is actually turned out to be 'privileged' op; I didn't want to
 * put it into privops.c because of '_eval' cr...(unbelievably evil stuff),
 * so I made it accessible but one is strongly cautioned NOT to call it
 * directly from any place.
 */
char *vsf_sysutil_real_path_from_absolute_path(const char *absolute_path, int access_kind);
