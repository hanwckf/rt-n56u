...
/* vsf_two_process_path_lookup()
 * PURPOSE
 * NASoC virtual path 2 real path conversion for two process model.
 * PARAMETERS
 * p_sess       - the current session object
 * p_path_str   - the path to convert.
 */
void vsf_two_process_path_lookup(struct vsf_session* p_sess, struct mystr* p_path_str, int access_kind);
