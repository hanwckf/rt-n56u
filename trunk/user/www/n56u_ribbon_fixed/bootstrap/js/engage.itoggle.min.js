...
(function($j){
    $j.fn.iState = function(new_state) {
        ...
        var $this = $j(this);
        var id = $this.attr('id');
        var cur_enabled = $this.children('.itoggle').hasClass('iTon');

        if(typeof $this === 'object')
        {
            console.info(cur_enabled + '; ' + new_state);

            if(new_state != cur_enabled)
            {
                var wasAttrDisabled = $this.children('input:first').attr('disabled') === 'disabled'
                if(wasAttrDisabled) $this.children('input:first').removeAttr('disabled');

                $j('#'+id + ' label').trigger('click');

                if(wasAttrDisabled) $this.children('input:first').attr('disabled', 'disabled');
            }
        }
