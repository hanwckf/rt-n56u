...
static void xhci_link_segments(struct xhci_segment *prev,
				struct xhci_segment *next, int link_trbs)
{
	u32 val;
	u64 val_64 = 0;
...
static struct xhci_segment *xhci_segment_alloc(void)
{
	...
	seg = (struct xhci_segment *)KSEG1ADDR(xhci_malloc(sizeof(struct xhci_segment)));
	BUG_ON(!seg);

	seg->trbs = (union xhci_trb *)KSEG1ADDR(xhci_malloc(SEGMENT_SIZE));
...
struct xhci_ring *xhci_ring_alloc(unsigned int num_segs, int link_trbs)
{
	...
	ring = (struct xhci_ring *)KSEG1ADDR(xhci_malloc(sizeof(struct xhci_ring)));
	BUG_ON(!ring);
		...
		*xhci_alloc_container_ctx(struct xhci_ctrl *ctrl, int type)
{
	...
	ctx = (struct xhci_container_ctx *)KSEG1ADDR(
	...
	ctx->bytes = (u8 *)KSEG1ADDR(xhci_malloc(ctx->size));
...
int xhci_alloc_virt_device(struct usb_device *udev)
{
	...
	ctrl->devs[slot_id] = (struct xhci_virt_device *)KSEG1ADDR(
					...
					struct xhci_hcor *hcor)
{
	...
	ctrl->dcbaa = (struct xhci_device_context_array *)KSEG1ADDR(
	...
	ctrl->erst.entries = (struct xhci_erst_entry *)KSEG1ADDR(
