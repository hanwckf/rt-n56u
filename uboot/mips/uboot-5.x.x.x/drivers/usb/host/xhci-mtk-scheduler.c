...
int mtk_xhci_scheduler_add_ep(int dev_speed, int is_in, int isTT, int ep_type, int maxp, int interval, int burst
	...
	printf("add_ep parameters, dev_speed %d, is_in %d, isTT %d, ep_type %d, maxp %d, interval %d, burst %d, mult %d, ep 0x%x, ep_ctx 0x%x, sch_ep 0x%x\n", dev_speed, is_in, isTT, ep_type, maxp , interval, burst, mult, (unsigned int)ep, (unsigned int)ep_ctx, (unsigned int)sch_ep);
	if(isTT && ep_type == USB_EP_INT && ((dev_speed == USB_SPEED_LOW) || (dev_speed == USB_SPEED_FULL))){
		...
		for(frame_idx=0; frame_idx<frame_interval; frame_idx++){
			printf("check tt_intr_bw interval %d, frame_idx %d\n", frame_interval, frame_idx);
		...
		for(frame_idx = 0; (frame_idx < interval) && !break_out; frame_idx++){
			for(repeat = max_repeat; repeat >= 0; repeat--){
				cur_bw = count_ss_bw(is_in, ep_type, maxp, interval, burst, mult, frame_idx
					, repeat, td_size);
				printf("count_ss_bw, frame_idx %d, repeat %d, td_size %d, result bw %d\n"
		...
		printf("final best idx %d, best repeat %d\n", best_bw_idx, best_bw_repeat);
		if(best_bw_idx == -1){
			return SCH_FAIL;
		}
		else{
	...
	if(ret == SCH_SUCCESS){
		...
		printf("[DBG] BPKTS: %x, BCSCOUNT: %x, BBM: %x\n", bPkts, bCsCount, bBm);
		printf("[DBG] BOFFSET: %x, BREPEAT: %x\n", bOffset, bRepeat);
		return SCH_SUCCESS;
	}
	else{
		return SCH_FAIL;
