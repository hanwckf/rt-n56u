...
{
	 const uint8_t HEADER_MAGIC[6] = { 0xFD, '7', 'z', 'X', 'Z', 0x00 };                                                                                                             
	if (!memeq(s->temp.buf, HEADER_MAGIC, HEADER_MAGIC_SIZE))
		return XZ_FORMAT_ERROR;

	if (crc32(0, s->temp.buf + HEADER_MAGIC_SIZE, 2)
			!= *(int *)(s->temp.buf + HEADER_MAGIC_SIZE + 2))
		return XZ_DATA_ERROR;
...
#else
	if (s->check_type > XZ_CHECK_CRC32) {
		printf("%d s->check_type=%d\n",__LINE__, s->check_type);
		return XZ_OPTIONS_ERROR;
	}
#endif
...
static enum xz_ret XZ_FUNC dec_stream_footer(struct xz_dec *s)
{
	...
	if (crc32(0, s->temp.buf + 4, 6) != *(int *)(s->temp.buf))
		return XZ_DATA_ERROR;
	 ...
	 */
	if ((s->index.size >> 2) != *(int *)(s->temp.buf + 4))
		return XZ_DATA_ERROR;
