...
#define SPICLK_DIV		SPICFG_SPICLK_DIV4 /* 166/4 = 41 MHz */
...
int spic_init(void)
{
	...
	ra_outl(RT2880_SPI0_CFG_REG, SPICFG_MSBFIRST | SPICFG_RXCLKEDGE_FALLING | SPICFG_TXCLKEDGE_FALLING | SPICLK_DIV);

	// set idle state
	ra_outl(RT2880_SPI0_CTL_REG, SPICTL_HIZSDO | SPICTL_SPIENA_HIGH);

	spi_wait_nsec = (8 * 1000 / ((128 / (SPICLK_DIV + 1)) )) >> 1;

	printf("Ralink SPI flash driver, SPI clock: %dMHz\n", (mips_bus_feq / 1000000) >> (SPICLK_DIV+1));
...
static int raspi_wait_ready(int sleep_ms)
{
	 ...
	 */
	for (count = 0;  count < ((sleep_ms+1)*10000); count++) {
